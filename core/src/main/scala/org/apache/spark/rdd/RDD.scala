...
abstract class RDD[T: ClassTag](
  ...
  def returnId(): Int = this.id//added by Ohno

  def registeredRDDArray(): Unit = {
    val start = System.nanoTime
    var flag = 0
    var num = 0
    var num2 = 0
    if (sc.count == 0) {
      println("error: no rdd is registered.")
    } else {
      for (i <- 0 to sc.count-1) {
        if (this.id == sc.rdd_collections(i).rdd_id) {
          flag = 1
          num = i
          num2 = sc.rdd_collections(i).rdd_id2
        }
      }
      if (flag == 1) {
        println("rdd.id(" + this.id + ").registeredArray is")
        sc.rdd_collections(num).rdd_type match {
          case "Int" => {
            for (i <- 0 to sc.int_rdd_collections(num2).length-1) {
              if (i != sc.int_rdd_collections(num2).length-1) {
                print(sc.int_rdd_collections(num2).rdd_array(i) + ", ")
              } else {
                println(sc.int_rdd_collections(num2).rdd_array(i))
              }
            }
          }
          case "Double" => {
            for (i <- 0 to sc.double_rdd_collections(num2).length-1) {
              if (i != sc.double_rdd_collections(num2).length-1) {
                print(sc.double_rdd_collections(num2).rdd_array(i) + ", ")
              } else {
                println(sc.double_rdd_collections(num2).rdd_array(i))
              }
            }
          }
          case "Byte" => {
            for (i <- 0 to sc.byte_rdd_collections(num2).length-1) {
              if (i != sc.byte_rdd_collections(num2).length-1) {
                print(sc.byte_rdd_collections(num2).rdd_array(i) + ", ")
              } else {
                println(sc.byte_rdd_collections(num2).rdd_array(i))
              }
            }
          }
          case "Int, Int" => {
            for (i <- 0 to sc.intint_rdd_collections(num2).length-1) {
              if (i != sc.intint_rdd_collections(num2).length-1) {
                print("(" + sc.intint_rdd_collections(num2).ks_rdd_array(i) + ", " + sc.intint_rdd_collections(num2).rdd_array(i) + "), ")
              } else {
                println("(" + sc.intint_rdd_collections(num2).ks_rdd_array(i) + ", " + sc.intint_rdd_collections(num2).rdd_array(i) + ")")
              }
            }
          }
          case "Int, String" => {
            for (i <- 0 to sc.intstring_rdd_collections(num2).length-1) {
              if (i != sc.intstring_rdd_collections(num2).length-1) {
                print("(" + sc.intstring_rdd_collections(num2).ks_rdd_array(i) + ", " + sc.intstring_rdd_collections(num2).rdd_array(i) + "), ")
              } else {
                println("(" + sc.intstring_rdd_collections(num2).ks_rdd_array(i) + ", " + sc.intstring_rdd_collections(num2).rdd_array(i) + ")")
              }
            }
          }
        }
      } else {
        println("error: rdd.id(" + this.id + ") isn't registered.")
      }
    }
    println("It takes " + (System.nanoTime - start) / 1e9 + "s")
  }//added by Ohno

  def initmallocInt(): Unit = {
    sc.int_rdd_collections(0).initmalloc()
  }//added by Ohno

  def initmallocInt2(): Unit = {
    sc.int_rdd_collections(0).initmalloc2()
  }//added by Ohno

  def initmallocInt3(): Unit = {
    sc.int_rdd_collections(0).initmalloc3()
  }//added by Ohno

  def initmallocDouble(): Unit = {
    sc.double_rdd_collections(0).initmalloc()
  }//added by Ohno

  def initmallocByte(): Unit = {
    sc.byte_rdd_collections(0).initmalloc()
  }//added by Ohno

  def initmallocIntInt(): Unit = {
    sc.intint_rdd_collections(0).initmalloc()
  }//added by Ohno

  def reduceIntByGPU(): Int = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      for (i <- 0 to sc.count-1) {
        if (this.id == sc.rdd_collections(i).rdd_id) {
          if (sc.rdd_collections(i).rdd_type != "Int") {
            typeflag = 0
          } else {
            flag = 1
            num = i
      ...
      if (flag == 1) {
        if (gpuflag != num) {
          if (gpuflag != 10000) {
            sc.rdd_collections(gpuflag).rdd_type match {
              case "Int" => {sc.int_rdd_collections(sc.rdd_collections(gpuflag).rdd_id2).finishGPU()}
              case "Double" => {sc.double_rdd_collections(sc.rdd_collections(gpuflag).rdd_id2).finishGPU()}
              case "Byte" => {sc.byte_rdd_collections(sc.rdd_collections(gpuflag).rdd_id2).finishGPU()}
              case "Int, Int" => {sc.intint_rdd_collections(sc.rdd_collections(gpuflag).rdd_id2).finishGPU()}
              case "Int, String" => {sc.intstring_rdd_collections(sc.rdd_collections(gpuflag).rdd_id2).finishGPU()}
            }
          }
          gpuflag = num
          sc.int_rdd_collections(num2).initializeGPU()
        }
        sum = sc.int_rdd_collections(num2).reduceByGPU()
        println("reduction completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Int.")
        } else {
          println("error: rdd.id(" + this.id + ") isn't registered.")
        }
      }
  ...
  def reduceDoubleByGPU(): Double = {
    ...
    if (sc.count == 0) {
      ...
      for (i <- 0 to sc.count-1) {
        if (this.id == sc.rdd_collections(i).rdd_id) {
          if (sc.rdd_collections(i).rdd_type != "Double") {
            typeflag = 0
          } else {
            flag = 1
            num = i
      ...
      if (flag == 1) {
        if (gpuflag != num) {
          ...
          gpuflag = num
          sc.double_rdd_collections(num2).initializeGPU()
        }
        sum = sc.double_rdd_collections(num2).reduceByGPU()
        println("reduction completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Double.")
        } else {
          println("error: rdd.id(" + this.id + ") isn't registered.")
        }
      }
  ...
  def dispersionIntByGPU(): Double = {
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        dispersion = sc.int_rdd_collections(num2).dispersionByGPU()
        println("dispersion completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Int.")
  ...
  def wordcountByGPU(): Int = {
    ...
    if (sc.count == 0) {
      ...
      for (i <- 0 to sc.count-1) {
        if (this.id == sc.rdd_collections(i).rdd_id) {
          if (sc.rdd_collections(i).rdd_type != "Byte") {
            typeflag = 0
          } else {
            flag = 1
            num = i
      ...
      if (flag == 1) {
        if (gpuflag != num) {
          ...
          gpuflag = num
          sc.byte_rdd_collections(num2).initializeGPU()
        }
        wordcount = sc.byte_rdd_collections(num2).wordcountByGPU()
        println("wordcount completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Byte.")
        } else {
          println("error: rdd.id(" + this.id + ") isn't registered.")
        }
      }
  ...
  def linecountByGPU(): Int = {
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        linecount = sc.byte_rdd_collections(num2).linecountByGPU()
        println("linecount completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Byte.")
  ...
  def wordconversionByGPU(f: Int = 0): RDD[Byte] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        sc.byte_rdd_collections(num2).wordconversionByGPU(f)
        println("converted rdd, and registered new rdd.")
        val rdd = sc.parallelize(sc.byte_rdd_collections(num2).result_array_byte)
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
        rdd
      } else {
        if (typeflag == 0) {
  ...
  def patternmatchByGPU(pattern_array:Array[Byte]): RDD[Byte] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        sc.byte_rdd_collections(num2).patternmatchByGPU(pattern_array)
        println("filtered rdd, and registered new rdd.")
        val rdd = sc.parallelize(sc.byte_rdd_collections(num2).result_array_byte)
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
        rdd
      } else {
  ...
  def maxIntByGPU(): Int = {
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        max = sc.int_rdd_collections(num2).maxminByGPU(0)
        println("max completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Int.")
  ...
  def minIntByGPU(): Int = {
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        min = sc.int_rdd_collections(num2).maxminByGPU(1)
        println("min completed.")
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match Int.")
...
//  def minIntByGPU(): Int = {
//    this.sortIntByGPU().first()
//  }//added by Ohno

//  def maxIntByGPU(): Int = {
//    this.sortIntByGPU(1).first()
//  }//added by Ohno

  def sortIntByGPU(f: Int = 0): RDD[Int] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        sc.int_rdd_collections(num2).sortByGPU2(f)
        println("sorted rdd, and registered new rdd.")
        val rdd = sc.parallelize(sc.int_rdd_collections(num2).result_array)
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
        rdd
      } else {
        if (typeflag == 0) {
  ...
  def sortDoubleByGPU(): RDD[Double] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        println("sorted rdd, and registered new rdd.")
        val rdd = sc.parallelize(sc.double_rdd_collections(num2).rdd_array)
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
        rdd
      } else {
        if (typeflag == 0) {
  ...
  def sortIntKeyByGPU(): RDD[(Int, Int)] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      for (i <- 0 to sc.count-1) {
        if (this.id == sc.rdd_collections(i).rdd_id) {
          if (sc.rdd_collections(i).rdd_type != "Int, Int") {
            typeflag = 0
          } else {
            flag = 1
            num = i
      ...
      if (flag == 1) {
        if (gpuflag != num) {
          ...
          gpuflag = num
          sc.intint_rdd_collections(num2).initializeGPU()
        }
        sc.intint_rdd_collections(num2).sortByGPU()
        println("sorted rdd, and registered new rdd.")
        val rdd1 = sc.parallelize(sc.intint_rdd_collections(num2).ks_result_array)
        val rdd2 = sc.parallelize(sc.intint_rdd_collections(num2).result_array)
        val rdd = rdd1.zip(rdd2)
        println("It takes " + (System.nanoTime - start) / 1e9 + "s")
        rdd
      } else {
        if (typeflag == 0) {
          println("error: rdd.id(" + this.id + ")'s rdd_type doesn't match (Int,Int).")
          sc.emptyRDD[(Int, Int)]
        } else {
          println("error: rdd.id(" + this.id + ") isn't registered.")
          sc.emptyRDD[(Int, Int)]
  ...
  def sortIntKeyByGPU2(): RDD[(Int, Int)] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        sc.intint_rdd_collections(num2).sortByGPU2()
        println("sorted rdd, and registered new rdd.")
        val rdd1 = sc.parallelize(sc.intint_rdd_collections(num2).ks_result_array)
        val rdd2 = sc.parallelize(sc.intint_rdd_collections(num2).result_array)
        val rdd = rdd1.zip(rdd2)
  ...
  def sortIntKeyByMultiGPU(): RDD[(Int, Int)] = {
    val start = System.nanoTime
    var flag = 0
    var typeflag = 1
    var num = 0
    ...
    if (sc.count == 0) {
      ...
      if (flag == 1) {
        ...
        sc.intint_rdd_collections(num2).sortByMultiGPU()
        println("sorted rdd, and registered new rdd.")
        val rdd1 = sc.parallelize(sc.intint_rdd_collections(num2).ks_result_array)
        val rdd2 = sc.parallelize(sc.intint_rdd_collections(num2).result_array)
        val rdd = rdd1.zip(rdd2)
