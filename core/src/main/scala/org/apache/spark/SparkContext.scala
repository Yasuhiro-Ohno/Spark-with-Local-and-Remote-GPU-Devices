...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  //added by Ohno 
  var rdd_collections = new Array[RDD_Collection](100)
  var count = 0
  var int_rdd_collections = new Array[Int_RDD_Collection](100)
  var int_count = 0
  var double_rdd_collections = new Array[Double_RDD_Collection](100)
  var double_count = 0
  var intint_rdd_collections = new Array[IntInt_RDD_Collection](100)
  var intint_count = 0
  var intstring_rdd_collections = new Array[IntString_RDD_Collection](100)
  var intstring_count = 0
  var byte_rdd_collections = new Array[Byte_RDD_Collection](100)
  var byte_count = 0
  ...
  def registerRDD(id:Int, array:Array[Int]): Unit = {
    val start = System.nanoTime
    var flag = 1
    if (int_count > 0) {
      for (i <- 0 to int_count-1) {
        if (id == int_rdd_collections(i).rdd_id) {
          flag = 0
        }
      }
    }
    if (flag == 1) {
      rdd_collections(count) = new RDD_Collection(id, array.length, "Int")
      int_rdd_collections(int_count) = new Int_RDD_Collection(id, array)
      println("registered.")
      println("count = " + count)
      println("int_count = " + int_count)
      println("int_rdd_collections(" + int_count + ").len = " + int_rdd_collections(int_count).len)
      count += 1
      int_count += 1
    } else {
      println("error: It is already registered.")
    }
    println("It takes(registerRDD(Int)) " + (System.nanoTime - start) / 1e9 + "s")
    println("Data Size = " + (array.length * 4) + "Byte")
  }//added by Ohno

  def registerRDD(id:Int, array:Array[Double]): Unit = {
    val start = System.nanoTime
    var flag = 1
    if (double_count > 0) {
      for (i <- 0 to double_count-1) {
        if (id == double_rdd_collections(i).rdd_id) {
          flag = 0
        }
      }
    }
    if (flag == 1) {
      rdd_collections(count) = new RDD_Collection(id, array.length, "Double")
      double_rdd_collections(double_count) = new Double_RDD_Collection(id, array)
      println("registered.")
      println("count = " + count)
      println("double_count = " + double_count)
      println("double_rdd_collections(" + double_count + ").len = " + double_rdd_collections(double_count).len)
      count += 1
      double_count += 1
    } else {
      println("error: It is already registered.")
    }
    println("It takes(registerRDD(Double)) " + (System.nanoTime - start) / 1e9 + "s")
    println("Data Size = " + (array.length * 8) + "Byte")
  }//added by Ohno

  def registerRDD(id:Int, array:Array[Byte]): Unit = {
    val start = System.nanoTime
    var flag = 1
    if (byte_count > 0) {
      for (i <- 0 to byte_count-1) {
        if (id == byte_rdd_collections(i).rdd_id) {
          flag = 0
        }
      }
    }
    if (flag == 1) {
      rdd_collections(count) = new RDD_Collection(id, array.length, "Byte")
      byte_rdd_collections(byte_count) = new Byte_RDD_Collection(id, array)
      println("registered.")
      println("count = " + count)
      println("byte_count = " + byte_count)
      println("byte_rdd_collections(" + byte_count + ").len = " + byte_rdd_collections(byte_count).len)
      count += 1
      byte_count += 1
    } else {
      println("error: It is already registered.")
    }
    println("It takes(registerRDD(Byte)) " + (System.nanoTime - start) / 1e9 + "s")
    println("Data Size = " + array.length + "Byte")//Java/Scala's Char is 2Byte but Byte is 1Byte
  }//added by Ohno

  def registerRDD(id:Int, ks_array:Array[Int], array:Array[Int]): Unit = {
    val start = System.nanoTime
    var flag = 1
    if (intint_count > 0) {
      for (i <- 0 to intint_count-1) {
        if (id == intint_rdd_collections(i).rdd_id) {
          flag = 0
        }
      }
    }
    if (flag == 1) {
      rdd_collections(count) = new RDD_Collection(id, ks_array.length, "Int, Int")
      intint_rdd_collections(intint_count) = new IntInt_RDD_Collection(id, ks_array, array)
      println("registered.")
      println("count = " + count)
      println("intint_count = " + intint_count)
      println("intint_rdd_collections(" + intint_count + ").len = " + intint_rdd_collections(intint_count).len)
      count += 1
      intint_count += 1
    } else {
      println("error: It is already registered.")
    ...
    println("Array Size = " + ks_array.length)
  }//added by Ohno


  def registerRDD(id:Int, ks_array:Array[Int], array:Array[String]): Unit = {
    val start = System.nanoTime
    var flag = 1
    if (intstring_count > 0) {
      for (i <- 0 to intstring_count-1) {
        if (id == intstring_rdd_collections(i).rdd_id) {
          flag = 0
        }
      }
    }
    if (flag == 1) {
      rdd_collections(count) = new RDD_Collection(id, ks_array.length, "Int, String")
      intstring_rdd_collections(intstring_count) = new IntString_RDD_Collection(id, ks_array, array)
      println("registered.")
      println("count = " + count)
      println("intstring_count = " + intstring_count)
      println("intstring_rdd_collections(" + intstring_count + ").len = " + intstring_rdd_collections(intstring_count).len)
      count += 1
      intstring_count += 1
    } else {
      println("error: It is already registered.")
  ...
  def registeredRDD(): Unit = {
    if (count == 0) {
      println("error: no rdd is registered.")
    } else {
      println(" No. | RDD.id | Array.length | Array.len | Type ")
      println("----------------------------------------------------")
      for (i <- 0 to count-1){
        println("  " +i+ "  |   " +rdd_collections(i).rdd_id+ "    |      " +rdd_collections(i).length+ "     |     " +rdd_collections(i).len+ "    |  " +rdd_collections(i).rdd_type+ " ")
      }
    }
  }//added by Ohno

/*  
  def sortIntKeyByMultiGPU(id:Int, ks_array:Array[Int], array:Array[Int]): Unit = {
    val start = System.nanoTime
    MultiGPU t1 = MultiGPU("thread 1")
    MultiGPU t2 = MultiGPU("thread 2")
    t1.start()
    t2.start()
    t1.join()
    t2.join()
    println("sortMultiGPU takes : " + (System.nanoTime - start) / 1e9 + "s")
  }//added by Ohno
*/
