...
object MultiGPUSortKey {
  def main(args: Array[String]) {
    val start = System.nanoTime
    val conf = new SparkConf().setAppName("MultiGPU SortKey")
...
//    rdd1.initmallocIntInt()
    ...
    //sortkey(1)
    val rdd2 = rdd1.sortIntKeyByMultiGPU()
...
//    println("It takes(initmalloc) " + (start4 - start3_2) / 1e9 + " s")
    println("It takes(SortKeyByMultiGPU) " + (start5 - start4) / 1e9 + " s")
